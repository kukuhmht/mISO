...
<br>
<div class="container">
<form action="<?php echo base_url('unit_kerja/upload_dokumen/update/'.$id_dokumen);?>" method="post">
<div class="row">
	<div class="col s12">
		...
		<div class="col s4 right" style="margin-top:20px;">
		  <select required id="id_jenispos" name="id_jenispos" class="browser-default validate">
			<?php foreach($ambil_jenis_pos as $row) { ?>
			  <option value="<?php echo $row->id_jenispos ?>" <?php echo ($row->id_jenispos == $id_jenispos ? 'selected' : '') ?> > <?php echo $row->jenis_pos ?> </option>
			<?php } ?>
		  </select>
		</div>
	  <div class="col s12">
		...
		<div class="input-field col s12">
		  <input required id="dokumen_path required" name="dokumen_path" value="<?php echo $dokumen_path ?>" type="text" placeholder="Contoh URL: https://drive.google.com/file/d/0BxuiQ-cxi2TYY2FYNjl0TE44dmM/preview" />
	...
	<div class="col s1 right">
		<button class="waves-effect waves-light btn green" type="submit" value="upload" name="upload"> simpan </button>
	</div>
</div>
</form>
</div>
